...
import {UploadFileDir} from '../../Tools/Utils';


var drop = false;
export default class JDataCloudList extends React.Component{
    ...
    _save = (fileList) => {
        HttpPost(URLS.SaveDataCloud, {fileList: fileList, diskType: 1}, '正在上传').then((response)=>{
            RRCToast.show(response.msg);
            if (response.result === 1){
                this._onHeaderRefresh();
            }
