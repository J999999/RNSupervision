...
class ReportOpinionSummary extends Component {
    ...
    renderSummaryData=(dataList)=>{
        ...
        for(let i  in dataList){
            ...
            if(item.replyList!=null){
                replyview.push(this.renderReplyList(item.replyList,item.recordType))
            }
            ...
            if(item.recordType == 2){ //工作汇报
                ...
                if(item.approveState!=0){ // 审批过
                    ...
                    approveView.push(
                        <View style={[styles.view,{justifyContent:'flex-end'}]}>
                            <Button style={styles.button} title="同意" onPress={()=>{
                                this.approvelWorkReport(item.id,"",1)
                            }} />
                            <Button style={styles.button} title="驳回" onPress={()=>{
                                this.reportId = item.id
                                this.editViewReject.show()
                            }} />
                            {this.renderReplyBtn(item.id,2) }
                        </View>
                    )
    ...
    renderReplyBtn=(id,type)=>{
        return <Button style={styles.button} title="回复" onPress={()=>{
            this.reportId = id
            this.type = type
            this.editViewReply.show()
        }} />
    }
        ...
        for(let i in list ){
            ...
            if(type == 1 ){ //批示
                let view = (
                    <View >
                        <View style={styles.view}>
                            <Text style={styles.titleInfo}>回复人：{reply.userName}</Text>
                            <Text style={styles.titleInfo}>回复时间：{reply.createTime}</Text>
                        </View>
                        <View style={styles.view}>
                            <Text style={styles.titleInfo}>回复内容：{reply.opinion}</Text>
                        </View>
                    </View>
                );
                ...
                let view = (
                    <View >
                        <View style={styles.view}>
                            <Text style={styles.titleInfo}>回复人：{reply.replyUserName}</Text>
                            <Text style={styles.titleInfo}>回复时间：{reply.replyTime}</Text>
                        </View>
                        <View style={styles.view}>
                            <Text style={styles.titleInfo}>回复内容：{reply.replyContent}</Text>
                        </View>
                    </View>
                );
                replyViews.push(view)
    ...
    render(){
        ...
        return (
            <View style = {styles.all}>
                ...
                <EditDialogWidget
                    ref={editView => this.editViewReject = editView}
                    inputText={this.state.suggestion}
                    titleTxt={'驳回原因'}
                    ensureCallback={(suggestion) => {

                        if(suggestion==undefined || suggestion==null ||  suggestion.replace(/(^\s*)|(\s*$)/g, "") == ""){
                            RRCToast.show('请输入内容')
                            return
                        }
                        this.approvelWorkReport(this.reportId,suggestion,99)
                    }}
                />
                <EditDialogWidget
                    ref={editView => this.editViewReply = editView}
                    inputText={this.state.suggestion}
                    titleTxt={'回复内容'}
                    ensureCallback={(suggestion) => {
                        ...
                        if(this.type ==1){
                            this.saveOpinionOrReply(this.reportId,suggestion)
                        }else{
                            this.replyWorkReport(this.reportId,suggestion)
                        }
                    }}
                />
            </View>
...
module.exports = ReportOpinionSummary
