...
var dutyStr = '';
var finishStr = '';
export default class PracticableDetail extends React.Component{
    static navigationOptions = ({navigation}) => ({
        title: '清单详情',
        headerRight: (<TouchableOpacity activeOpacity={.5}
                                        onPress={()=>{navigation.state.params.rightOnPress()}}>
            <Text style={{color: '#fff', marginRight: 10*unitWidth}}>{'落实情况'}</Text>
        </TouchableOpacity>)
    });
    _ClickHeaderRightAction = () => {
        this.props.navigation.navigate('PracticableLuoShi', {LS: this.state.luoShiData})
    };
    constructor (){
        super ();
        this.state = {
