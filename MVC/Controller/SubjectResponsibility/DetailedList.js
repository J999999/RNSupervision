...
export default class DetailedList extends React.Component{
    ...
    _clickCellAction = (item) => {
        this.props.navigation.navigate('DetailedListDetail', {id: item.id});
    };
    // _searchAction = (info) => {
    //     search = {};
    //     let searchArr = [];
    ...
    //         if (i.dutyTypes)
    //             search['dutyTypes'] = i.dutyTypes.join(',');
    //         if (i.progress)
    //             search['progress'] = i.progress.join(',');
    //         if (i.approveStates)
    //             search['approveStates'] = i.approveStates.join(',');
    //     });
    //     this._onHeaderRefresh();
    // };
}
