...
import {arrayToObj, getExpandedMap, getPredecessorsMap, traverseTree} from '../../Tools/Utils';
...
export default class TreeSelect extends React.Component {
    ...
    componentWillMount() {
        ...
        const { predecessorsOfNodeMap, ownedLeafNodesMap, allNodesMap } = getPredecessorsMap(treeData);

        let leafNodesSelectedKeysMap = {};
        let predecessorsSelectedKeysMap = {};
        if (multiple) {
            leafNodesSelectedKeysMap = arrayToObj(value, ownedLeafNodesMap);
        ...
        if (treeDefaultExpandAll) {
            ...
            expandedMap = getExpandedMap(treeDefaultExpandedKeys, predecessorsOfNodeMap);
        } else {
    ...
    onSelect = (node, check) => {
        ...
        if (multiple) {
            hasChildren && traverseTree(children, (node) => {
                const { key, children } = node;
    ...
    render() {
       ...
       this. initData()
        return (
            <ScrollView>
                {this.props.treeData ? this.renderTree(this.props.treeData, true) :null}
            </ScrollView>
        );
    }
}
